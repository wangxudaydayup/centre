$(function () {
        ...
        success: function (res) {
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    $('.cover img').attr('src', res.data.show_image)

                    $('.accessory span').text(res.data.source_file)

                    // 文件尺寸渲染
                    var sizeArr = res.data.source_size.replace(/px/g, '').split(' * ')
                    // console.log(sizeArr);
                    $('input[name=length]').val(sizeArr[0])
                    $('input[name=width]').val(sizeArr[1])

                    // 自定义分辨率渲染
                    if (!$('.layui-form-radio>div').text().split('dpi').includes(res.data.source_dpi.replace('dpi', ''))) {
                        $('#resolution i:last').click()
                        $('#resolution input:last').val(res.data.source_dpi.replace('dpi', ''))
                    }
                    ...
                    $('#fileCover').after(`<input type="hidden" name="downloadurl" value="${res.data.downloadurl}">
    ...
    $('.accessory button').on('click', function () {
        ...
        $('#fileCover').click().on('change', function () { // 监听 input[type='file'] 的 change 事件
            ...
            if (filePath) {
                $('.accessory span').html(fileName) // 如果选择了文件，则将文件名渲染至页面相应位置
                var fd2 = new FormData($('#uploadList')[0])
                fd2.append('file', $('#fileCover')[0].files[0])
                // 上传源文件
                $.ajax({
                    method: 'POST',
                    ...
                    success: function (res) {
                        ...
                        $('input[name=downloadurl]').val(res.data.downloadurl)
    ...
    form.on('submit(formDemo)', function (e) {
        layer.confirm('是否确认当前操作?', {
            ...
            if ($('#resolution input[type="text"]').val()) {
